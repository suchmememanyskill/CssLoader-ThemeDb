...
files = [
    f
    for f in listdir("./themes")
    if isfile(join("./themes", f)) and f.endswith(".json")
]
...
class MegaJson:
    def __init__(self):
        self.megaJson = requests.get(
            "https://github.com/suchmememanyskill/CssLoader-ThemeDb/releases/download/1.1.0/themes.json"
        ).json()
...
class RepoReference:
    def __init__(self, json: dict, path: str):
        ...
        self.previewImagePath = (
            json["preview_image_path"] if "preview_image_path" in json else None
        )
        self.downloadUrl = ""
        self.repo = None
        self.id = binascii.hexlify(
            hashlib.sha256(
                f"{self.repoUrl}.{self.repoSubpath}.{self.repoCommit}".encode("utf-8")
            ).digest()
        ).decode("ascii")
        ...
        result = subprocess.run(
            ["git", "log", "-1", "--pretty=%ci", path], capture_output=True
        )
    ...
    def override(self):
        self.target = (
            self.overrides["target"] if "target" in self.overrides else self.target
        )
    ...
    def existsInMegaJson(self) -> bool:
        ...
        return self.megaJsonEntry is not None
    ...
    def toDict(self):
        ...
        if self.megaJsonEntry is not None:

            def possiblyReturnMegaJsonStuff(attribute: str, original):
                return (
                    self.megaJsonEntry[attribute]
                    if attribute in self.megaJsonEntry
                    else original
                )
            ...
            manifestVersion = possiblyReturnMegaJsonStuff(
...
class Repo:
    ...
    def get(self):
        ...
        if self.repoReference.repoUrl != "LOCAL":
            ...
            subprocess.run(
                ["git", "clone", self.repoReference.repoUrl, tempDir.name], check=True
            )
    ...
    def read(self, json: dict):
        ...
        self.author = (
            str(json["author"]) if "author" in json else None
        )  # This isn't required by the css loader but should be for the theme store
        self.target = (
            str(json["target"]) if "target" in json else self.target
        )  # This isn't used by the css loader but used for sorting instead
        self.manifestVersion = (
            int(json["manifest_version"]) if "manifest_version" in json else 1
        )
    ...
    def verify(self):
        ...
        releasePath = (
            join(self.themePath, "release.json")
            if os.path.exists(join(self.themePath, "release.json"))
            else "release.json"
        )
        ...
        if "dependencies" in self.json:
            if self.manifestVersion < 3:  # Dependencies got introduced in manifest v3
                raise Exception(
                    "A v3+ Patch was detected but a v2 or v1 manifest was provided"
                )
        ...
        if "patches" in self.json:
            for x in self.json["patches"]:
                ...
                if (
                    "default" not in patch and self.manifestVersion < 3
                ):  # Default is only required on pre-3 manifest
                ...
                if "values" in patch:  # V2 patch
                    ...
                    ):  # Manifest version needs to be set to 2 or above to support v2 patches
                        raise Exception(
                            "A v2+ Patch was detected but a v1 manifest was provided"
                        )
                    ...
                    ):  # Manifest version needs to be set to 1 or below to support v1 patches
                        raise Exception(
                            "A v1 patch was detected but a v2+ manifest was provided"
                        )
                    ...
                    if isinstance(values[y], dict):
                        for z in values[y]:
                            if not os.path.exists(join(self.themePath, z)):
                                raise Exception(
                                    f"Patch '{x}' contains css that does not exist"
                                )

                            if not z.endswith(".css"):
                                raise Exception(
                                    f"Path '{x}' contains a non-css file '{z}'!"
                                )
                ...
                if "components" in patch:
                    if (
                        self.manifestVersion < 3
                    ):  # Components got introduced in manifest v3
                        raise Exception(
                            "A v3+ Patch was detected but a v2 or v1 manifest was provided"
                        )
                    ...
                    for z in patch["components"]:
                        ...
                        for i in items:
                            if i not in z:
                                raise Exception(
                                    f"Field {y} not found in component of '{x}'"
                                )
...
class DiscordWebhooks:
    ...
    def send(self, repo: Repo):
        ...
        try:
            ...
            embed = DiscordEmbed(
                title=repo.name, description=repo.target, color="03b2f8"
            )
...
for x in themes:
    if len([y for y in themes if y["name"] == x["name"]]) > 1:
        raise Exception(
            f"Multiple themes with the same name detected in the repository! Name is '{x['name']}'"
        )
